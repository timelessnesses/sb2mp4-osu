...
parser = argparse.ArgumentParser(description='sb2mp4 (stole from walavouchey repo (gib meh excuse pzlpzlpzpzlpz uwu))')

parser.add_argument('osb_folder', help='The song folder that have osb file.')
parser.add_argument('-diff', '--difficulty', help='The difficulty path that you want to render.', required=False)
parser.add_argument('-w', '--width', help='The width of the video. (Default 1920)', required=False)
parser.add_argument('-he', '--height', help='The height of the video. (Default 1080)', required=False)
parser.add_argument('-f', '--fps', help='The fps of the video. (Default 30)', required=False)
parser.add_argument('-o', '--output', help='The output file name. (Default output.mp4)', required=False)
parser.add_argument('-s', '--start-time', help='The start time of the beatmap. (Default 0)', required=False)
parser.add_argument('-d', '--duration', help='The duration of the beatmap. (Default end)', required=False)
...
diff = parser.parse_args().difficulty if parser.parse_args().difficulty is not None else None
if diff:
...
sb = Storyboard(files, ParseStoryboard(files, osbfile, diff, inDiff=True), (frameW, frameH))
...
duration = pydub.utils.to_ms(parser.parse_args().duration) if parser.parse_args().duration else None
